...
import rospy
import rospkg
from speech.msg import SpeechRecognition
...
class vosk_sr():
    def __init__(self):
        ...
        rospack.list()
        package_path = rospack.get_path('speech')
        
        model_path = '/models/'
        model_dir = package_path + model_path
        model = 'vosk-model-small-es-0.42' #change the name of the model to match the downloaded model's name
        ...
        self.tts_status_topic = rospy.get_param('~tts_status_topic','speech/tts/status')
        speech_recognition_text_topic = rospy.get_param('~speech_recognition_text_topic','speech/recognition/text')
        self.tts_status = False

        # ROS node initialization

        # self.pub_vosk = rospy.Publisher('speech/recognition/vosk_result',SpeechRecognition, queue_size=10)
        self.pub_final = rospy.Publisher(speech_recognition_text_topic,String, queue_size=10)
        # self.pub_partial = rospy.Publisher('speech/recognition/partial_result',String, queue_size=10)
    ...
    def tts_status_listenner(self):
        rospy.Subscriber(self.tts_status_topic, Bool, self.tts_get_status)
